...
@Composable
fun OrderListScreen(navController: NavController, orderViewModel: OrderViewModel){
    val apiService = ApiClient.retrofit.create(ApiService::class.java)
    val coroutineScope = rememberCoroutineScope()
    var listOrders by remember { mutableStateOf<List<OrderEntity>>(emptyList()) }
            ...
            Column(
                ...
                if (listOrders.isEmpty()) {
                    Box(
                        ...
                        Text("Falha ao carregar pedidos")
                    }
                } else {
                    LazyColumn(
                        modifier = Modifier.fillMaxSize(),
                        ...
                        items(listOrders) {
                                listOrders -> OrderCard(listOrders) {
                            orderViewModel.setOrder(listOrders)
                            navController.navigate(Screens.OrdersDetailRoute.route)
                        }
                        }
                    }
                }
...
fun OrderCard(order: OrderEntity, onClick: () -> Unit) {
    Card(
        ...
        Column(
            ...
            Text(text = "Código: ${order.code}", style = MaterialTheme.typography.bodyMedium)
            Text(text = "Status: ${order.status}", style = MaterialTheme.typography.bodyLarge)
            Text(text = "Valor: ${order.totalprice}", style = MaterialTheme.typography.bodyLarge)
        }
    }
}